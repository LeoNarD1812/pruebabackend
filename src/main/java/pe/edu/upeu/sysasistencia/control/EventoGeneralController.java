...
public class EventoGeneralController {
    ...
    @GetMapping("/ciclo/{ciclo}/programa/{programaId}")
    public ResponseEntity<List<EventoGeneralDTO>> findByCicloYPrograma(
            @PathVariable String ciclo,
            @PathVariable Long programaId
    ) {
        List<EventoGeneralDTO> list = eventoMapper.toDTOs(
                eventoService.findByCicloYPrograma(ciclo, programaId)
        );
        return ResponseEntity.ok(list);
    }
